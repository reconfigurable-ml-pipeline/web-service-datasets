...
TWITTER_LOGS_DIRECTORY = config("twitter_trace_logs_directory", default=CURRENT_DIR)
if TWITTER_LOGS_DIRECTORY[-1] == "/":
    TWITTER_LOGS_DIRECTORY = TWITTER_LOGS_DIRECTORY[:-1]
...
parser.add_argument(
    "--days",
    help="Day(s) of the month of the dataset to process. Whether specify a day (e.g. 5) or a range (e.g 2-10)",
    required=True,
    type=str
)
parser.add_argument(
...
if __name__ == "__main__":
    ...
    with open(f"{CURRENT_DIR}/workload.txt", "w") as f:
        for count in all_requests:
            f.write(f"{count} ")
    os.system(
        f"tar -jcvf {CURRENT_DIR}/workload.tbz2 "
        f"{CURRENT_DIR}/workload.txt"
    )
